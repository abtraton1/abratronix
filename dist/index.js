...
// server/feedScheduler.ts
import { spawn } from "child_process";
import path from "path";
import fs from "fs";
import cron from "node-cron";
var PROJECT_ROOT = path.resolve(process.cwd());
var FETCHER_SCRIPT = path.join(PROJECT_ROOT, "fetcher", "fetch.py");
var FEED_OUTPUT = path.join(PROJECT_ROOT, "client", "public", "data", "feed.json");
var isRunning = false;
var lastRunAt = null;
var lastRunStatus = null;
var lastRunDuration = null;
function runFetcher() {
  if (isRunning) {
  ...
  return new Promise((resolve) => {
    ...
    console.log(`[FeedScheduler] Starting feed refresh at ${(/* @__PURE__ */ new Date()).toISOString()}`);
